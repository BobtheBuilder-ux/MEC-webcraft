...
export const getAvailableTimeSlots = async (date: string, timezone?: string): Promise<TimeSlot[]> => {
...
export const scheduleAppointment = async (details: AppointmentDetails): Promise<{ success: boolean, appointmentId?: string }> => {
  try {
    ...
    const availableSlots = await getAvailableTimeSlots(slotDate, details.timezone);
    
    const isSlotAvailable = availableSlots.some(
      slot => slot.startTime === details.startTime && 
    ...
    const response = await axios.post(
      `${CALENDAR_API_URL}/calendars/${CALENDAR_ID}/events`,
      {
