...
const ProjectDetails = () => {
  ...
  return (
    <>
      {project && (
        <Helmet>
          <title>{`${project.title} - Web Development Project | MEC-webcraft`}</title>
          <meta name="description" content={project.description} />
          <meta name="keywords" content={`${project.tags.join(', ')}, ${project.tools?.join(', ')}, web development, portfolio`} />
          <meta name="robots" content="index, follow" />
          <meta property="og:type" content="website" />
          <meta property="og:title" content={`${project.title} - Development Case Study`} />
          <meta property="og:description" content={project.description} />
          {project.images?.[0] && <meta property="og:image" content={project.images[0]} />}
          <link rel="canonical" href={`https://mecwebcraft.com/projects/${project.id}`} />
        </Helmet>
      )}
      <div className="min-h-screen bg-gray-50">
        <Navbar items={navItems} onNavClick={() => {}} />
